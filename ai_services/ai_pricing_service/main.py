...
class PricingRequest(BaseModel):
    base_price: float = Field(..., gt=0, description="Базовая цена за час, ₽")
    ...
    hour: Optional[int] = Field(
        None, ge=0, le=23, description="Час суток (0–23, локальное время парковки)"
    )
    dow: Optional[int] = Field(
        None, ge=0, le=6, description="День недели (0=понедельник, 6=воскресенье)"
    )
    use_ml: bool = Field(
        False,
        description=(
